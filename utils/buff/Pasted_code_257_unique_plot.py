...
def main():
    ...
    env_id = "PerturbMountainCarContinuous-v0"
    metric_list = ["nominal", "local_mean", "local_min"]
    num_trials = 5

    # perturbation_types = ["none", "gravity", "mass", "length"]
    perturbation_types = ["gravity", "power"]

    for perturbation_type in perturbation_types:
        # 여러 알고리즘의 경로를 리스트로 정의
        algo_config_list = [
            # ("ppo_avg", f"logs/tuned/{env_id}/{perturbation_type}/1024"),
            # ("fedsp_pg_ppo", f"logs/tuned/{env_id}/{perturbation_type}/1024"),
            # ("fedsp_pg_ppo_paper_aligned", f"logs/tuned/{env_id}/{perturbation_type}/1024"),

            ("ppo_avg", f"logs/tuned/{env_id}/20/{perturbation_type}/8"),
            ("fedsp_pg_ppo", f"logs/tuned/{env_id}/20/{perturbation_type}/8"),
            ("fedsp_pg_ppo_paper_aligned", f"logs/tuned/{env_id}/20/{perturbation_type}/8/0.0003"),
            ("fedsp_pg_ppo_paper_aligned", f"logs/tuned/{env_id}/20/{perturbation_type}/8/0.001"),
            ("fedsp_pg_ppo_paper_aligned", f"logs/tuned/{env_id}/20/{perturbation_type}/8/0.003"),
            # ("fedsp_pg_ppo_paper_aligned", f"logs/tuned/{env_id}/1/{perturbation_type}/8/0.01"),
        ]

        # plot 저장 root
        plot_root_path = f"plots/frl_eh/tuned/{env_id}/20/{perturbation_type}/8"

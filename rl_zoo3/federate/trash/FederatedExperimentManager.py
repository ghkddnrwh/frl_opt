...
class FederatedExperimentManager(ExperimentManager):
    ...
    perturbation strengths around zero mean and evaluating both:
    ...
    def _sample_client_noises(self) -> list[float]:
        if self.perturb_noise_type is None or self.perturb_noise_std <= 0.0:
            return [0.0 for _ in range(self.num_clients)]

        rng = np.random.default_rng(self.seed)
        samples = rng.normal(loc=self.perturb_noise_mean, scale=self.perturb_noise_std, size=self.num_clients)
    ...
    def _init_eval_history(self) -> None:
        ...
        self.eval_history = {
            ...
            "perturb_noise_mean": float(self.perturb_noise_mean),
            "perturb_noise_std": float(self.perturb_noise_std),
            "perturb_noise_clip": np.nan if self.perturb_noise_clip is None else float(self.perturb_noise_clip),
        }
    ...
    def _save_eval_npz(self) -> None:
        ...
        payload = {
            ...
            "perturb_noise_mean": np.asarray(self.eval_history["perturb_noise_mean"], dtype=np.float32),
            "perturb_noise_std": np.asarray(self.eval_history["perturb_noise_std"], dtype=np.float32),
            "perturb_noise_clip": np.asarray(self.eval_history["perturb_noise_clip"], dtype=np.float32),
        }
    ...
    def setup_experiment(self) -> tuple[BaseAlgorithm, dict[str, Any]] | None:
        ...
        clip_value = hyperparams.pop("perturb_noise_clip", None)
        self.perturb_noise_clip = None if clip_value is None else float(clip_value)

        self.eval_local_episodes = int(hyperparams.pop("eval_local_episodes", 0))
        self.eval_nominal_episodes = int(hyperparams.pop("eval_nominal_episodes", 0))
        ...
        if self.perturb_noise_std < 0.0:
            raise ValueError("perturb_noise_std must be >= 0")
        if self.perturb_noise_clip is not None and self.perturb_noise_clip <= 0.0:
            raise ValueError("perturb_noise_clip must be > 0 when provided")
        ...
        if self.continue_training:
            ...
            self.server_model = self._build_model(self._base_hyperparams, env_kwargs_override=self._build_nominal_env_kwargs())
        ...
        if self.verbose > 0 and self.perturb_noise_type is not None:
            print(
                ...
                f"mean={self.perturb_noise_mean} std={self.perturb_noise_std} "
                f"samples={self.client_noise_values}"
            )
    ...
    def _evaluate_clients(self, model: BaseAlgorithm, round_idx: int) -> None:
        ...
        for client_idx, client_model in enumerate(self.clients):
            if self.eval_local_episodes > 0:
                ...
                model.logger.record(f"frl_eval/local/client_{client_idx}_mean", float(local_mean))
                model.logger.record(f"frl_eval/local/client_{client_idx}_std", float(local_std))
            ...
            if self.eval_nominal_episodes > 0 and self.nominal_eval_env is not None:
                ...
                model.logger.record(f"frl_eval/nominal/client_{client_idx}_mean", float(nominal_mean))
                model.logger.record(f"frl_eval/nominal/client_{client_idx}_std", float(nominal_std))

        local_means = [mean for mean, _ in local_results]
        nominal_means = [mean for mean, _ in nominal_results]

        if local_means:
            model.logger.record("frl_eval/local/mean_across_clients", float(np.mean(local_means)))
            model.logger.record("frl_eval/local/min_across_clients", float(np.min(local_means)))
            model.logger.record("frl_eval/local/max_across_clients", float(np.max(local_means)))

        if nominal_means:
            model.logger.record("frl_eval/nominal/mean_across_clients", float(np.mean(nominal_means)))
            model.logger.record("frl_eval/nominal/min_across_clients", float(np.min(nominal_means)))
            model.logger.record("frl_eval/nominal/max_across_clients", float(np.max(nominal_means)))

        model.logger.record("frl_eval/round", round_idx)
        ...
        try:
            while remaining_timesteps > 0:
                ...
                for client_idx, client_model in enumerate(self.clients):
                    client_algo = cast(FederatedAlgorithmMixin, client_model)
                    if self.verbose > 1:
                        client_noise = self.client_noise_values[client_idx] if client_idx < len(self.client_noise_values) else 0.0

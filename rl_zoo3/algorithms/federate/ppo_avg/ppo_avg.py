...
class PPOAvg(FederatedAlgorithmMixin, PPO):
    ...
    Overall synchronization is configurable via ``sync_mode``:
      - ``fedavg``: run the normal federated upload -> average -> broadcast cycle.
      - ``full_local``: keep every client fully independent for the whole run.
        No policy/critic parameters or VecNormalize statistics are uploaded,
        averaged, or applied back to clients.

    Within ``sync_mode="fedavg"``, critic synchronization is configurable via
    ``critic_sync_mode``:
    ...
    ``sync_mode="full_local"`` overrides the finer-grained synchronization
    settings and behaves as if ``critic_sync_mode="local"`` and
    ``vecnormalize_sync_mode="none"`` were selected.

    Important VecNormalize detail (federated modes only):
    ...
    valid_sync_modes: tuple[str, ...] = ("fedavg", "full_local")
    ...
    def __init__(self, *args, **kwargs):
        # ``sync_mode="fedavg"`` preserves the original PPOAvg behavior.
        # ``sync_mode="full_local"`` disables all cross-client model/statistic
        # sharing while keeping the existing federated training loop available
        # for scheduling, logging, and per-client evaluation.
        self.sync_mode = self._normalize_sync_mode(kwargs.pop("sync_mode", "fedavg"))
        ...
        # Full-local is intentionally strict: actor, critic, and normalization
        # statistics all remain private to each client.
    ...
    def _normalize_sync_mode(cls, mode: str) -> str:
        """Normalize overall federation mode aliases.

        Modes:
          - fedavg: perform the normal federated upload/average/broadcast cycle.
          - full_local: keep every client's actor, critic, optimizer, and
            VecNormalize statistics client-local for the whole run.
        """
        ...
        if normalized not in cls.valid_sync_modes:
            raise ValueError(
                f"Unsupported sync_mode={mode!r}. "
                f"Choose one of {cls.valid_sync_modes}."
            )
        return normalized
    ...
    def get_upload_payload(self) -> FederatedPayload:
        # In full-local mode, clients intentionally do not upload policy/value
        # parameters or VecNormalize statistics. Only non-training metadata is
        # returned so the existing manager can keep round/timestep accounting.
        if self.sync_mode == "full_local":
            return {
                "modules": {},
                "vecnormalize": None,
    ...
    def aggregate_uploads(
        ...
        sync_modes = {
            cls._normalize_sync_mode(str(upload.get("sync_mode", "fedavg")))
            for upload in uploads
        }
        if len(sync_modes) != 1:
            raise ValueError(f"Mixed sync_mode values are not supported in one aggregation: {sync_modes}")
        ...
        # Strict full-local mode: there are deliberately no trainable tensors or
        # normalization statistics to average. Returning an empty payload keeps
        # the manager API stable without creating any cross-client information
        # flow through model parameters/statistics.
        if sync_mode == "full_local":
            ...
            return {
                ...
                "meta": {
                    ...
                    "client_timesteps": client_timesteps,
                    "full_local": True,
                },
            }
    ...
    def apply_global_payload(self, payload: FederatedPayload, mix_weight: float = 1.0) -> None:
        payload_sync_mode = self._normalize_sync_mode(
            str(payload.get("sync_mode", payload.get("meta", {}).get("sync_mode", self.sync_mode)))
        )
        if payload_sync_mode != self.sync_mode:
            raise ValueError(
                f"Client sync_mode={self.sync_mode!r} does not match "
                f"payload sync_mode={payload_sync_mode!r}."
            )

        # Full-local clients never consume a global model/statistics payload.
        # Importantly, return before _set_module_states(), VecNormalize updates,
        # and optimizer-state clearing so the local PPO trajectory is untouched.
        if self.sync_mode == "full_local":
            meta = payload.get("meta", {})
            self._last_federated_metrics = {
                "server/num_clients": float(meta.get("num_clients", 0)),
                "server/ppo_avg/full_local": 1.0,
                "server/ppo_avg/model_delta_norm": 0.0,
                "server/ppo_avg/critic_local": 1.0,
                "server/ppo_avg/critic_fedavg": 0.0,
                "server/ppo_avg/vecnormalize_none": 1.0,
                "server/ppo_avg/vecnormalize_obs": 0.0,
                "server/ppo_avg/vecnormalize_reward": 0.0,
                "server/ppo_avg/vecnormalize_obs_reward": 0.0,
            }
            return

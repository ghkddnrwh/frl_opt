...
class FedAMPOLocalActorUpdateGroupPPO(FederatedAlgorithmMixin, PPO):
    """Fed-AMPO-LocalActorUpdate-GroupPPO: Fed-AMPO-PPO with configurable local actor update frequency and an optional capped dual simplex.

    Paper-level round:
      1. the server broadcasts the shared actor theta,
      2. each client collects local trajectories with that round-start actor,
      3. for each rollout, the fixed n_epochs PPO budget is kept intact; the
         resulting minibatch-gradient evaluations are partitioned as evenly as
         possible across U local actor updates.  Each segment accumulates actor
         gradients while applying critic updates, then performs one actual actor step
         using the *mean* gradient of that segment,
      4. independently, the client accumulates a server/global actor-gradient signal
         across all minibatches using actor_gradient_mode ("mean" or "cumulative")
         and uploads it with a training-stream completed-episode return estimate J_k,
      5. the server applies theta <- theta + eta_pi * sum_k lambda_k g_k,
      6. on the configured slow dual timescale, the server applies
         lambda <- Proj_Delta(lambda - eta_lambda * dual_signal(J)).

    Local actor steps use ``local_actor_lr``.  When ``local_actor_lr=None``
    (default), they use the current PPO learning-rate schedule value, i.e. the same
    learning rate used by the PPO optimizer/critic.

    Local actor steps are always based on the mean gradient of their assigned
    minibatch segment, regardless of actor_gradient_mode.  The actor_gradient_mode
    setting is reserved for the gradient uploaded to / applied by the server.

    With local_actor_updates=1, all server-gradient evaluations are still performed at
    the round-start actor before the one local actor step, so the server update exactly
    reduces to the original Fed-AMPO-PPO mean/cumulative gradient update.  For U>1,
    n_epochs itself is unchanged.
    Instead, the total number of actor-gradient/minibatch evaluations generated by
    those n_epochs is divided across the U actor steps, so the total data passes and
    critic-update budget per rollout stay fixed.
    ...
    Actor-update geometry diagnostics are server-side measurements only; they do not
    modify the actor update.  They report client/aggregate cosine alignment,
    high-lambda conflict rates, and lambda-weighted update contribution magnitudes.
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.local_actor_updates = int(kwargs.pop("local_actor_updates", 1))
        if self.local_actor_updates <= 0:
            raise ValueError("local_actor_updates must be positive.")

        # By default, local actor steps use the same current PPO learning-rate
        # schedule value as the critic optimizer.  Set local_actor_lr explicitly
        # only when the local actor step size should be decoupled from PPO.
        raw_local_actor_lr = kwargs.pop("local_actor_lr", None)
        self.local_actor_lr = None if raw_local_actor_lr is None else float(raw_local_actor_lr)
        if self.local_actor_lr is not None and self.local_actor_lr <= 0.0:
            raise ValueError("local_actor_lr must be positive or None.")
        ...
        total_actor_batches = self._total_actor_gradient_batches_per_rollout()
        if self.local_actor_updates > total_actor_batches:
            raise ValueError(
                "local_actor_updates cannot exceed the total number of actor-gradient "
                "minibatch evaluations in one rollout. "
                f"Got local_actor_updates={self.local_actor_updates}, "
                f"total_actor_batches={total_actor_batches}, n_epochs={self.n_epochs}, "
                f"n_steps={self.n_steps}, n_envs={self.n_envs}, batch_size={self.batch_size}."
            )
    ...
    def _mix_modules(old: FederatedModules, new: FederatedModules, mix_weight: float) -> FederatedModules:
        if mix_weight >= 1.0:
            return FedAMPOLocalActorUpdateGroupPPO._clone_modules(new)
    ...
    def _current_ppo_learning_rate(self) -> float:
        """Current PPO LR used by the shared SB3 optimizer (critic by default)."""
    ...
    def _current_local_actor_lr(self) -> float:
        """LR for actual local actor steps.  Defaults to the current PPO LR."""
        if self.local_actor_lr is not None:
            return float(self.local_actor_lr)
        return self._current_ppo_learning_rate()
    ...
    def _actor_minibatches_per_epoch(self) -> int:
        """Number of PPO minibatches produced by one full pass over the rollout buffer."""
        total_size = int(self.n_steps) * int(self.n_envs)
        batch_size = int(self.batch_size or total_size)
        if batch_size <= 0:
            raise ValueError(f"batch_size must be positive, got {batch_size}.")
        return (total_size + batch_size - 1) // batch_size

    def _total_actor_gradient_batches_per_rollout(self) -> int:
        """Total actor-gradient evaluations under the fixed n_epochs budget."""
        return int(self.n_epochs) * self._actor_minibatches_per_epoch()

    def _local_actor_gradient_batch_schedule(self) -> tuple[int, ...]:
        """Split total minibatch-gradient evaluations across U actor updates.

        ``local_actor_updates`` remains the external control.  Internally we first
        compute the total number of actor-gradient evaluations

            n_epochs * minibatches_per_epoch

        and partition that count as evenly as possible across the requested actor
        updates.  For example, 80 evaluations with U=4 gives (20, 20, 20, 20),
        while 82 evaluations with U=4 gives (21, 21, 20, 20).
        """
        updates = int(self.local_actor_updates)
        total_batches = self._total_actor_gradient_batches_per_rollout()
        if updates <= 0 or updates > total_batches:
            raise ValueError(
                "Require 1 <= local_actor_updates <= total actor-gradient batches, "
                f"got local_actor_updates={updates}, total_actor_batches={total_batches}."
            )
        base, remainder = divmod(total_batches, updates)
        return tuple(base + (1 if idx < remainder else 0) for idx in range(updates))

    def _update_local_actor_and_critic_multistep(self) -> tuple[int, FederatedModules]:
        """Run the fixed n_epochs budget and perform U actual local actor steps.

        Two actor-gradient accumulators are intentionally kept separate:

        * ``local_gradient`` is reset after every local actor step and is ALWAYS
          averaged over that block before updating the local actor.  Therefore local
          actor movement is independent of ``actor_gradient_mode``.
        * ``server_gradient`` spans the whole rollout.  At the end it follows
          ``actor_gradient_mode``: mean divides by the total actor-gradient batch
          count, while cumulative keeps the raw sum.  This is the signal uploaded for
          the global/server actor update.

        Critic updates remain exactly once per minibatch, as in the original code.

        Returns ``(num_actor_batches, server_gradient)``.
        """
        ...
        module_name = self.federated_actor_module_name
        batch_schedule = self._local_actor_gradient_batch_schedule()
        actor_step_lr = self._current_local_actor_lr()

        local_gradient = self._zero_like_actor_state()
        server_gradient = self._zero_like_actor_state()
        block_index = 0
        batches_in_block = 0
        total_batches = 0

        self.policy.set_training_mode(True)
        learning_rate = self._current_ppo_learning_rate()
        ...
        for _ in range(int(self.n_epochs)):
            for rollout_data in self._iter_rollout_minibatches():
                ...
                # Compute one actor-gradient sample at the current local actor.
                ...
                for name, parameter in actor_params.items():
                    if parameter.grad is not None and name in local_gradient[module_name]:
                        direction = -parameter.grad.detach().cpu()
                        local_gradient[module_name][name] += direction
                        server_gradient[module_name][name] += direction

                # Preserve the original critic behavior: one actual critic optimizer
                # step for every minibatch used to estimate actor directions.
                self.policy.optimizer.zero_grad(set_to_none=True)
                if critic_params:
                ...
                # Local actor updates ALWAYS use the mean gradient of the current
                # block, regardless of the global/server actor_gradient_mode.
                if batches_in_block == batch_schedule[block_index]:
                    for key in local_gradient[module_name].keys():
                        local_gradient[module_name][key] /= float(batches_in_block)

                    actor_current = self._get_actor_state()
                    actor_next = self._add_scaled_modules(
                        actor_current,
                        local_gradient,
                        actor_step_lr,
                    )
                    self._set_actor_state(actor_next)

                    block_index += 1
                    batches_in_block = 0
                    if block_index < len(batch_schedule):
                        local_gradient = self._zero_like_actor_state()

        if total_batches != sum(batch_schedule):
            raise RuntimeError(
                "Actor-gradient batch accounting mismatch: "
                f"processed={total_batches}, scheduled={sum(batch_schedule)}."
            )
        if block_index != len(batch_schedule) or batches_in_block != 0:
            raise RuntimeError(
                "Local actor-update schedule was not fully consumed: "
                f"completed_updates={block_index}, expected={len(batch_schedule)}, "
                f"residual_batches={batches_in_block}."
            )

        # Only the SERVER/GLOBAL gradient follows actor_gradient_mode.
        if self.actor_gradient_mode == "mean":
            for key in server_gradient[module_name].keys():
                server_gradient[module_name][key] /= float(total_batches)

        self._ampo_last_num_actor_batches = total_batches
        return total_batches, server_gradient
    ...
    def federated_local_update(self, local_steps: int, **kwargs) -> None:
        ...
        # Every rollout starts from the same round-start actor.  This preserves the
        # original Fed-AMPO data-collection policy.  For U=1, the uploaded global
        # gradient exactly follows the original mean/cumulative semantics even when
        # local_steps spans multiple rollouts.  Critic updates are kept across rollouts.
        actor_anchor = self._get_actor_state()
        critic_before = self._get_critic_state() if self.critic_sync_mode == "actor_like" else None
        actor_step_lr = self._current_local_actor_lr()

        collected_steps = 0
        completed_episode_returns: list[float] = []
        accumulated_actor_delta: FederatedModules | None = None
        accumulated_server_gradient: FederatedModules | None = None
        num_rollouts = 0
        num_actor_batches = 0

        while collected_steps < target_steps:
            # Restore the round-start actor before collecting each rollout.
            # The critic remains locally trained, exactly as in the original code.
            self._set_actor_state(actor_anchor)
            completed_episode_returns.extend(self._collect_one_rollout())

            # Keep n_epochs unchanged.  Internally, all minibatch actor-gradient
            # evaluations produced by those epochs are divided as evenly as possible
            # across exactly local_actor_updates actual actor steps.
            rollout_num_actor_batches, rollout_server_gradient = self._update_local_actor_and_critic_multistep()
            num_actor_batches += rollout_num_actor_batches

            if accumulated_server_gradient is None:
                accumulated_server_gradient = self._clone_modules(rollout_server_gradient)
            else:
                for module_name, module_state in accumulated_server_gradient.items():
                    for key in module_state.keys():
                        module_state[key] += rollout_server_gradient[module_name][key]

            rollout_actor_delta = self._subtract_modules(self._get_actor_state(), actor_anchor)
            if accumulated_actor_delta is None:
                accumulated_actor_delta = self._clone_modules(rollout_actor_delta)
            else:
                for module_name, module_state in accumulated_actor_delta.items():
                    for key in module_state.keys():
                        module_state[key] += rollout_actor_delta[module_name][key]

            collected_steps += self.n_steps * self.n_envs
            num_rollouts += 1

        assert accumulated_actor_delta is not None
        assert accumulated_server_gradient is not None
        for module_state in accumulated_actor_delta.values():
            for key in module_state.keys():
                module_state[key] /= float(num_rollouts)
        for module_state in accumulated_server_gradient.values():
            for key in module_state.keys():
                module_state[key] /= float(num_rollouts)

        # Local delta is retained for diagnostics/full_local behavior only.
        self._ampo_last_actor_delta = accumulated_actor_delta
        # This is the actual gradient signal used by the federated server update.
        # Its mean/cumulative semantics are independent of local actor steps.
        self._ampo_last_actor_gradient = accumulated_server_gradient
        ...
        if self.sync_mode == "full_local":
            actor_updated = self._add_scaled_modules(actor_anchor, accumulated_actor_delta, 1.0)
            ...
            # Federated clients restore the persistent actor to the round-start
            # anchor.  Local actor steps only shape where later minibatch gradients
            # are evaluated; the server update uses the separately accumulated
            # mean/cumulative global actor-gradient signal.
            self._set_actor_state(actor_anchor)
    ...
    def get_upload_payload(self) -> FederatedPayload:
        ...
        payload: FederatedPayload = {
            ...
            "local_actor_updates": int(self.local_actor_updates),
            "local_actor_lr": self.local_actor_lr,
            "effective_local_actor_lr": float(self._current_local_actor_lr()),
            "local_actor_gradient_batch_schedule": self._local_actor_gradient_batch_schedule(),
    ...
    def aggregate_uploads(
        ...
        actor_gradient_mode = cls._normalize_actor_gradient_mode(
            str(_shared_value("actor_gradient_mode", "mean"))
        )
        local_actor_updates = int(_shared_value("local_actor_updates", 1))
        local_actor_lr = _shared_value("local_actor_lr", None)
        effective_local_actor_lr = float(_shared_value("effective_local_actor_lr", 3e-4))
        ...
        payload: FederatedPayload = {
            ...
            "local_actor_updates": local_actor_updates,
            "local_actor_lr": local_actor_lr,
            "effective_local_actor_lr": effective_local_actor_lr,
    ...
    def apply_global_payload(self, payload: FederatedPayload, mix_weight: float = 1.0) -> None:
        ...
        payload_local_actor_updates = int(payload.get("local_actor_updates", 1))
        if payload_local_actor_updates != self.local_actor_updates:
            raise ValueError(
                f"Server local_actor_updates={self.local_actor_updates} does not match "
                f"payload {payload_local_actor_updates}."
            )
        payload_local_actor_lr = payload.get("local_actor_lr", None)
        if payload_local_actor_lr is not None:
            payload_local_actor_lr = float(payload_local_actor_lr)
        if payload_local_actor_lr != self.local_actor_lr:
            raise ValueError(
                f"Server local_actor_lr={self.local_actor_lr!r} does not match "
                f"payload {payload_local_actor_lr!r}."
            )
        effective_local_actor_lr = float(
            payload.get("effective_local_actor_lr", self._current_local_actor_lr())
        )
        ...
        aggregated_actor_gradient = self._weighted_module_sum(
            actor_before,
            client_actor_gradients,
            self.lambda_weights,
        )

        # Global/server update is controlled by server_actor_lr and by the uploaded
        # actor_gradient_mode signal.  It is intentionally independent of the local
        # actor-step LR and the local actor deltas.
        actor_lr = self._current_server_actor_lr()
        actor_updated = self._add_scaled_modules(actor_before, aggregated_actor_gradient, actor_lr)
        if mix_weight < 1.0:
        ...
        gradient_diagnostics = self._compute_gradient_diagnostics(
            ...
            aggregated_gradient=aggregated_actor_gradient,
        ...
        self._record_server_metrics(
            ...
            aggregated_gradient_norm=self._module_l2_norm(aggregated_actor_gradient),
    ...
    def _record_server_metrics(
        ...
        *,
        ...
        metrics: dict[str, float] = {
            ...
            "server/ampo/local_actor_lr": float(effective_local_actor_lr),
            "server/ampo/local_actor_updates": float(self.local_actor_updates),
    ...
    def _get_rms_state(rms: Any) -> RunningMeanStdState | dict[str, RunningMeanStdState]:
        if isinstance(rms, Mapping):
            return {key: FedAMPOLocalActorUpdateGroupPPO._get_rms_state(value) for key, value in rms.items()}
        return {
    ...
    def _set_rms_state(rms: Any, state: RunningMeanStdState | dict[str, RunningMeanStdState]) -> None:
        if isinstance(rms, Mapping):
            for key, value in state.items():
                if key in rms:
                    FedAMPOLocalActorUpdateGroupPPO._set_rms_state(rms[key], value)
            return
    ...
    def _clone_state(state: Any) -> Any:
        ...
        if isinstance(state, Mapping):
            return {key: FedAMPOLocalActorUpdateGroupPPO._clone_state(value) for key, value in state.items()}
        return state
...
# Canonical local-actor-update name. local_actor_updates=1 reproduces
# the original Fed-AMPO actor update exactly. Values >1 keep n_epochs fixed and
# partition its total minibatch-gradient evaluations across multiple actor steps.
# FedAMPOLocalActorUpdatePPO = FedAMPOLocalActorUpdateGroupPPO

# # Backward-compatible names.
# FedAMPOLocalActorPPO = FedAMPOLocalActorUpdateGroupPPO
# FedAMPOMultiStepPPO = FedAMPOLocalActorUpdateGroupPPO
FedAMPOGroupPPO = FedAMPOLocalActorUpdateGroupPPO
# FedAMPOWorstGroupPPO = FedAMPOLocalActorUpdateGroupPPO
# FedAMPOPPO = FedAMPOLocalActorUpdateGroupPPO
# FedAMPPO = FedAMPOLocalActorUpdateGroupPPO

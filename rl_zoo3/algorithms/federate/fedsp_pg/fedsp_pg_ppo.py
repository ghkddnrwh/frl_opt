...
class FedSPPGPPO(FederatedAlgorithmMixin, PPO):
    ...
    Client side:
      ...
      3. run PPO-style minibatch loops over all local rollouts while keeping
         the temporary local actor updates inside the round,
      4. after all local rollouts, measure the final actor delta and restore
         the local actor to the round-start global actor,
      5. send the round-level actor delta and Monte-Carlo return estimate to the server.
    ...
    def _snapshot_optimizer_param_state(
        self,
        parameters: Sequence[th.nn.Parameter],
    ) -> dict[th.nn.Parameter, dict[str, Any] | None]:
        """Snapshot optimizer slots for selected parameters only.

        FedSP-PG uses temporary actor updates to estimate an actor delta. The
        actor parameters are restored after the estimate, so their Adam/RMSProp
        slots should also be restored. Critic optimizer slots are intentionally
        left untouched because the critic update is real client-local training.
        """
        snapshot: dict[th.nn.Parameter, dict[str, Any] | None] = {}
        for parameter in parameters:
            if parameter in self.policy.optimizer.state:
                snapshot[parameter] = deepcopy(self.policy.optimizer.state[parameter])
            else:
                snapshot[parameter] = None
        return snapshot

    def _restore_optimizer_param_state(
        self,
        snapshot: dict[th.nn.Parameter, dict[str, Any] | None],
    ) -> None:
        for parameter, state in snapshot.items():
            if state is None:
                ...
                self.policy.optimizer.state[parameter] = state
    ...
    def _train_one_rollout_joint_actor_critic(self) -> None:
        """Run one PPO-style joint actor/critic update on the current rollout.

        The actor is intentionally *not* restored inside this method. This lets
        all rollouts in one federated local stage behave like ordinary local PPO
        training: rollout t+1 is collected by the actor produced after rollout t.

        The round-level actor delta is measured later in ``federated_local_update``
        as ``actor_after_all_rollouts - actor_before_round``. After measuring
        that delta, the local actor is restored so that the server remains the
        only place where the global actor step is committed.
        """
        actor_params = self._actor_named_parameters()
        if not actor_params:
            raise RuntimeError("Could not identify actor parameters for FedSP-PG PPO.")
        ...
        critic_params = self._critic_named_parameters()
        if not critic_params:
            # Keep the method usable for custom policies that do not expose
            # value_net-style names. In that case, it degenerates to an
            # actor-only local update used to form the round-level actor delta.
            critic_params = {}
        ...
        update_params = {**actor_params, **critic_params}
        update_param_names = set(update_params.keys())
        ...
        for _ in range(self.n_epochs):
            for rollout_data in self._iter_rollout_minibatches():
                ...
                # Keep all non-selected parameters fixed. In particular, this
                # prevents accidental updates to parameters that are neither in
                # the synchronized actor set nor in the client-local critic set.
                for name, parameter in self.policy.named_parameters():
                    if name not in update_param_names:
                        parameter.grad = None

                th.nn.utils.clip_grad_norm_(list(update_params.values()), self.max_grad_norm)
                self.policy.optimizer.step()
    ...
    def federated_local_update(self, local_steps: int, **kwargs) -> None:
        """Collect all local rollouts, then upload one round-level actor delta.
        ...
        Unlike the previous version, this method does not compute and average a
        separate actor delta after each rollout. Instead, the local actor is
        allowed to keep its temporary PPO updates throughout the whole local
        stage. After all requested rollouts finish, the uploaded actor delta is

            actor_after_all_rollouts - actor_before_round.

        This is closer to PPOAvg/FedAvg semantics: with uniform aggregation and
        common round-start parameters, averaging final deltas is equivalent to
        averaging final client parameters. The local actor is restored after the
        delta is measured so that only the server commits the global actor step.
        """
        del kwargs
        ...
        actor_optimizer_snapshot = self._snapshot_optimizer_param_state(tuple(actor_params.values()))
        ...
        while collected_steps < target_steps:
            ...
            self._train_one_rollout_joint_actor_critic()

            returns.append(rollout_return)
            collected_steps += self.n_steps * self.n_envs

        actor_after_round = self._get_actor_state()
        self._fedsp_last_gradient = self._subtract_static_modules(actor_after_round, actor_before_round)
        self._fedsp_last_return = float(np.mean(returns)) if returns else 0.0
        ...
        # FedSP-PG uploads the actor update direction and lets the server apply
        # the global actor step. Restore only actor parameters and actor optimizer
        # slots; keep critic parameters and critic optimizer state as real local
        # critic training.
        self._set_actor_state(actor_before_round)
        self._restore_optimizer_param_state(actor_optimizer_snapshot)
        self.policy.optimizer.zero_grad(set_to_none=True)
    ...
    def apply_global_payload(self, payload: FederatedPayload, mix_weight: float = 1.0) -> None:
        ...
        aggregated_grad_norm = self._module_l2_norm(aggregated_grad)
        ...
        actor_step_norm = self._module_delta_l2_norm(updated_actor, actor_state)
        ...
        lambda_after = np.asarray(self.lambda_weights, dtype=np.float64)
        metrics: dict[str, float] = {
            "server/num_clients": float(num_clients),
            "server/fedsp/return_mean": float(np.mean(client_returns)),
            "server/fedsp/return_std": float(np.std(client_returns)),
            "server/fedsp/return_min": float(np.min(client_returns)),
            "server/fedsp/return_max": float(np.max(client_returns)),
            "server/fedsp/client_grad_norm_mean": float(np.mean(client_gradient_norms)),
            "server/fedsp/client_grad_norm_std": float(np.std(client_gradient_norms)),
            "server/fedsp/client_grad_norm_min": float(np.min(client_gradient_norms)),
            "server/fedsp/client_grad_norm_max": float(np.max(client_gradient_norms)),
            "server/fedsp/aggregated_grad_norm": float(aggregated_grad_norm),
            "server/fedsp/actor_step_norm": float(actor_step_norm),
            "server/fedsp/dual_lr": float(self.dual_lr),
            "server/fedsp/dual_uniform": float(self._uses_fixed_uniform_lambda()),
            "server/fedsp/critic_local": float(self.critic_sync_mode == "local"),
            "server/fedsp/critic_fedavg": float(self.critic_sync_mode == "fedavg"),
            "server/fedsp/critic_actor_like": float(self.critic_sync_mode == "actor_like"),
            "server/fedsp/lambda_entropy": self._simplex_entropy(lambda_after),
            "server/fedsp/lambda_min": float(np.min(lambda_after)),
            "server/fedsp/lambda_max": float(np.max(lambda_after)),
            "server/fedsp/effective_clients": self._effective_num_clients(lambda_after),
            "server/fedsp/lambda_delta_norm": float(np.linalg.norm(lambda_after - lambda_before)),
        }
        for client_idx, (client_return, lambda_weight, grad_norm) in enumerate(
            ...
            metrics[f"server/fedsp/client_{client_idx}/return"] = float(client_return)
            metrics[f"server/fedsp/client_{client_idx}/lambda"] = float(lambda_weight)
            metrics[f"server/fedsp/client_{client_idx}/grad_norm"] = float(grad_norm)
...
# #       1. aggregate client actor gradients with the current dual weights lambda,
# #       2. perform one global actor gradient-ascent step,
# #       3. update lambda by projected gradient descent on the simplex.

# #     Important design choice:
# #       Stable-Baselines3 stores actor and critic inside one ActorCriticPolicy.
# #       Therefore, all synchronization/update helpers below explicitly filter the
# #       policy state_dict so that only actor parameters are synchronized globally.
# #       Critic/value-network parameters remain client-local.
# #     """
...
# #         # Broadcast path: clients receive the updated global actor only. Their
# #         # critic/value parameters are intentionally preserved as local state.
# #         if "actor_state" in payload and "client_actor_gradients" not in payload:
# #             incoming_actor = payload["actor_state"]
# #             if mix_weight < 1.0:
# #                 current = self._get_actor_state()
...
# #         for key, value in actor_state[module_name].items():
# #             if th.is_floating_point(value):
# #                 updated_actor[module_name][key] = value + aggregated_grad[module_name][key]
# #             else:
...
# #         # Dual update: lambda <- Proj_Delta(lambda - eta_lambda * normalized(J)).
# #         dual_signal = self._normalized_dual_update_signal(client_returns)
# #         self.lambda_weights = project_to_simplex(self.lambda_weights - self.dual_lr * dual_signal)
# #         self._fedsp_num_clients_hint = num_clients

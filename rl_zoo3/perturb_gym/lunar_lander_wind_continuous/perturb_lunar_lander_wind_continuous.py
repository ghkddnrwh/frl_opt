...
class PerturbLunarLanderWindContinuous(gym.Env, EzPickle):
    r"""
    ## Description
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def __init__(
        ...
        if isinstance(noise, float):
            ...
            assert all(isinstance(n, float) for n in noise), "all elements in noise list must be floats"
            assert all(-1 < n < 1 for n in noise), "all noise values must be in the range (-1, 1)"
        else:
            raise TypeError("noise must be a float or a list of floats")
        
        env_name = "PerturbLunarLanderWindContinuous-v3"
        ...
        if noise_type is not None:
            if noise_type in noise_map[env_name]:
                target_attrs = noise_map[env_name][noise_type]

                # noise가 리스트인 경우
                if isinstance(noise, list):
                    if len(noise) != len(target_attrs):
                        raise ValueError(
                            f"Length of noise ({len(noise)}) must match number of target attributes ({len(target_attrs)}) for noise_type '{noise_type}'"
                        )
                    for attr, n in zip(target_attrs, noise):
                        print("Noise : ", n)
                        setattr(self, attr, getattr(self, attr) * (1 + n))
        ...
        print("wind_power : ", self.wind_power)
        print("turbulence_power : ", self.turbulence_power)
        ##############################################################################################################
        EzPickle.__init__(
            self,
            render_mode,
        ...
        assert (
            -12.0 < self.gravity and self.gravity < 0.0
        ), f"gravity (current value: {self.gravity}) must be between -12 and 0"
        # self.gravity = gravity

        if 0.0 > self.wind_power or self.wind_power > 20.0:
            gym.logger.warn(
                f"wind_power value is recommended to be between 0.0 and 20.0, (current value: {self.wind_power})"
            )
        # self.wind_power = wind_power

        if 0.0 > self.turbulence_power or self.turbulence_power > 2.0:
            gym.logger.warn(
                f"turbulence_power value is recommended to be between 0.0 and 2.0, (current value: {self.turbulence_power})"
            )
        # self.turbulence_power = turbulence_power
